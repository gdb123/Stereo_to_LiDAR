function calib = loadCalibrationCamObject(filename)
...
for cam=1:100
  R_rect_ = readVariable(fid,'R0_rect',3,3);
  P_rect_ = readVariable(fid,['P' num2str(cam-1,'%01d')],3,4);
  ...
  if  ~isempty(R_rect_) && ~isempty(P_rect_)
